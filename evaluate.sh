...
if [ ! -f "${MEASUREMENTS_FILE%.txt}.out" ]; then
  ...
  echo ""
  echo "  ./calculate_average_thomaswue.sh > ${MEASUREMENTS_FILE%.txt}.out"
  echo ""
  exit 1
fi
...
for fork in "$@"; do
  ...
  source_name=${fork//-/_}
  if grep -F "theUnsafe" -q ./src/main/java*/dev/morling/onebrc/CalculateAverage_$source_name.java ; then
  ...
  echo -n "| [link](https://github.com/gunnarmorling/1brc/blob/main/src/main/java/dev/morling/onebrc/CalculateAverage_$source_name.java)" >> $leaderboard_temp_file
